...
#     decodedidtoken = jwt.decode(idtoken.encode(
#         "utf-8"), channel, algorithms=["HS256"], options={"verify_signature": False})  # 解碼 id_token
...
#     if miuscrow:
#         UserMusicList._meta.db_table = uid
#         userplaylistrow = UserMusicList.objects.using(
#             "usermusic").values_list('playlist', flat=True).distinct()
#     else:
...
#     # setting
#     UserMusicList._meta.db_table = uid
#     settingrow = UserSetting.objects.using(
#         "user").filter(UID_SETTING=uid).all()[0]
#     setting = {
...
#     printcolorhaveline("green", "save session "+str(
#         request.session["user_data"])+str(request.session["user_playlist"]), "#")
...
#                 printcolorhaveline("green", "add ", music_ID,
#                                    "=> ", music_list, " ")
#                 # 查询数据库中是否已存在相同的 music_ID
#                 UserMusicList._meta.db_table = uid
#                 row = UserMusicList.objects.using("usermusic").filter(
#                     music_ID=music_ID, playlist=music_list).count()
#                 if row == 0:  # 不存在则插入新数据
#                     UserMusicList._meta.db_table = uid
...
#                     UserMusicList._meta.db_table = uid
#                     UserMusicList.objects.using("usermusic").filter(
#                         music_ID=music_ID).update(favorite=True)
#             success = True
...
#         UserMusicList._meta.db_table = uid
#         row = UserMusicList.objects.using("usermusic").filter(
#             playlist=music_list).order_by('-created_at').values_list('music_ID', flat=True)
...
#                 UserMusicList._meta.db_table = uid
#                 UserMusicList.objects.using("usermusic").filter(
#                     playlist=music_list, music_ID=music_ID).delete()
#                 # 如果是我的最愛，則將favorite設為false
#                 if music_list == 1:
#                     UserMusicList._meta.db_table = uid
#                     UserMusicList.objects.using("usermusic").filter(
#                         music_ID=music_ID).update(favorite=False)
#             success = True
#         except Exception as e:
#             printcolorhaveline("fail", e, "-")
#             success = False
...
#         UserMusicList._meta.db_table = uid
#         row = UserMusicList.objects.using("usermusic").exclude(
#             playlist='我的最愛').values_list('playlist', flat=True).distinct().all()
#         if row:
#             check = row
#         else:
#             check = None
...
#     # 生成 Google 登入連結
#     url = f"https://accounts.google.com/o/oauth2/v2/auth?scope={scope}&access_type={access_type}&include_granted_scopes={include_granted_scopes}&response_type={response_type}&state={state}&redirect_uri={urlgoogle}&client_id={client_id}"
...
#     # 检查code和state是否存在，并且state的值与之前保存在会话中的oauth_state相匹配。
...
#         # 調用 base 函式進行處理 将获取到的用户信息传递给它进行处理。
