...
def search_music(file_name, query, results_list) -> list : 
    ...
    results = sorted(song_list, key=lambda song: song_search(song, query), reverse=True)
    results = results[:20]
    # print(type(results))
    # print(results)
...
def new_query(request, query) : 
    ...
    try :
        ...
        results = sorted(search_list, key=lambda song: song_search(song, query), reverse=True)
